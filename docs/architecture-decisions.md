...
"Intentamos usar dbt para crear las tablas Gold como Delta. Pero dbt-spark genera un CREATE OR REPLACE TABLE AS SELECT. En nuestra combinación de Spark 3.5.8 + Delta 3.3.2, esa operación pasa por DataSource V2 y requiere que la tabla soporte TRUNCATE. Delta no declara esa capacidad en ese camino, por lo que Spark falla antes de crear/reemplazar la tabla. Probamos varias configuraciones y el error fue idéntico, así que no parece un problema de configuración nuestra. En lugar de seguir peleando con esa incompatibilidad, dejamos Delta para Bronze/Silver, donde sí es importante por streaming y concurrencia, y usamos Parquet para Gold."
